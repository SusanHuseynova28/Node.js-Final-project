...
export default function AdminPanel() {
  const { data: collections, mutate } = useSWR<Collection[]>('http://localhost:3001/api/collections', fetcher);
  const [editingItem, setEditingItem] = useState<Collection | null>(null);
  const [newItem, setNewItem] = useState({ title: '', price: '', description: '', imageUrl: '', hoverImageUrl: '' });
    ...
    <>
      ...
      <div className="container mx-auto py-8">
        ...
        <div className="text-center mb-8 flex justify-between gap-4 p-6 ">
          ...
          />
          <button onClick={() => setIsCreateModalOpen(true)} className="bg-black text-white p-2 h-[40px]">
            Add New Collection
          </button>
        </div>
            ...
            {filteredCollections.map((collection) => (
              <tr key={collection._id} className="hover:bg-gray-50">
                <td className="border px-4 py-2 text-center">
                  <img src={collection.imageUrl} alt={collection.title} className="h-20 w-20 object-cover mx-auto" />
                </td>
                <td className="border px-4 py-2 text-center">{collection.title}</td>
                <td className="border px-4 py-2 text-center">${collection.price}</td>
                <td className="border px-4 py-2 text-center">{collection.description}</td>
        ...
        {isCreateModalOpen && (
          <div className="fixed inset-0 flex items-center justify-center z-50 bg-gray-800 bg-opacity-75">
            <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
              ...
              />
              {previewImage && (
                <img src={previewImage} alt="Preview" className="h-20 w-20 object-cover mx-auto mb-4" />
              )}
              <div className="flex justify-end">
                <button onClick={addNewItem} className="bg-black text-white px-4 py-2 mr-2">Save</button>
                <button onClick={() => setIsCreateModalOpen(false)} className="bg-gray-500 text-white px-4 py-2">Cancel</button>
              </div>
            </div>
          </div>
        )}
        ...
        {isEditModalOpen && editingItem && (
          <div className="fixed inset-0 flex items-center justify-center z-50 bg-gray-800 bg-opacity-75">
            <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
              ...
              />
              <div className="flex justify-end">
                <button onClick={saveItem} className="bg-black text-white px-4 py-2 mr-2">Save</button>
                <button onClick={() => setIsEditModalOpen(false)} className="bg-gray-500 text-white px-4 py-2">Cancel</button>
              </div>
            </div>
          </div>
        )}
